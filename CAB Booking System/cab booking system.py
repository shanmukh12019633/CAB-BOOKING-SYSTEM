...
def validate():
    ...
    for i in users:
        uname, passw, name, _, _, _ = i.split('|')
        ...
        if '@' in email and '.com' in email:
            ...
            for i in users:
                if usr in i or email in i:
                    messagebox.showerror("Validation Error", "Username/Email Already Exists")
                    flag = 1

            if flag == 0:
                with open("users.txt", 'a') as f:
                    f.write(f"{usr}|{pwd}|{name}|{email}\n")
...
def RegisterPage():
    ...
    labels = ["Your Name", "Username:", "Password:", "Email:"]
    entries = [name_entry, user_entry, pass_entry, email_entry]

    for i, label in enumerate(labels):
        canvas.create_text(150, 200 + i * 40, text=label, fill='black', font=("Arial", 13, 'bold'))
        canvas.create_window(300, 200 + i * 40, window=entries[i])

    Reg_button = tk.Button(canvas, text="Register", width=10, fg="white", bg="#088", font=("Arial", 13, 'bold'), command=RegisterNow)
    canvas.create_window(200, 500, window=Reg_button)

    Login_button = tk.Button(canvas, text="Login", fg="white", bg="#607B8B", bd=0, font=("Arial", 13, 'bold'), command=loginpage)
    canvas.create_window(320, 500, window=Login_button)
...
def forgotpass():
    ...
    pwd = pass_entry.get()
    seciq = var.get()
    secians = security_answer.get()

    if all([usr, pwd, secians]):
        flag = 0
        with open("users.txt", 'r') as f:
            users = list(f)
        ...
        messagebox.showerror("Error", "Empty Entries")
...
def submitreview():
    ...
    review = feed_entry.get("1.0", 'end-1c')
    with open('reviews.txt', 'a') as f:
        f.write(logged_user + '|' + str(var.get()) + '|' + review + '\n')
...
def feedpage():
    ...
    feedback_button = tk.Button(canvas, text="booking page", width=15, fg="white", bg="Red", bd=0, font=("Arial", 13, 'bold'), command=gotocpage)
    canvas.create
